...
class Form extends React.Component {
  ...
  onSubmit = async (event) => {
    ...
    // if the user has input a preferred alias, then we use it. If not, we generate one for them
    var generatedKey = nanoid(5);
    var generatedURL = "shrinkthelink.com/" + generatedKey; // ternary instead???

    if (this.state.preferredAlias !== "") {
      generatedKey = this.state.preferredAlias;
      generatedURL = "shrinkthelink.com/" + this.state.preferredAlias;
    }
  ...
  // Goes through our form to make sure everything looks good
  validateInput = async () => {
    var errors = [];
    ...
    if (this.state.longURL.length === 0) {
      ...
      errorMessages["longURL"] =
        "Please enter a valid URL in the form of https://www...";
    }
      ...
      <div className="container">
        <form autoComplete="off">
          ...
          <div className="form-group">
            ...
            <div
              className={
                this.hasError("suggestedAlias") // preferredAlias?
                  ? "text-danger"
                  : "visually-hidden"
              }
            >
